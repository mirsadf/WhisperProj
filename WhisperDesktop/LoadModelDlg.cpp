...
LRESULT LoadModelDlg::OnInitDialog( UINT nMessage, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if( FAILED( hr ) )
	{
		...
		::MessageBox( m_hWnd, text, L"Nije moguće učitati model", MB_OK | MB_ICONWARNING );
		return TRUE;
	}
...
LRESULT LoadModelDlg::OnBrowse( UINT, INT, HWND, BOOL& bHandled )
{
	...
	if( !getOpenFileName( m_hWnd, L"Odaberite datoteku GGML modela", L"Binary files (*.bin)\0*.bin\0\0", path ) )
		return 0;
...
{
	reportError( m_hWnd, message, L"Nije moguće učitati model" );
	return 0;
}

LRESULT LoadModelDlg::validationError( LPCTSTR message, HRESULT hr )
{
	reportError( m_hWnd, message, L"Nije moguće učitati model", hr );
	return 0;
}
...
LRESULT LoadModelDlg::OnOk( UINT, INT, HWND, BOOL& bHandled )
{
	modelPath.GetWindowText( path );
	if( path.GetLength() <= 0 )
		return validationError( L"Molimo odaberite model GGML fajla" );

	{
		CAtlFile file;
		HRESULT hr = file.Create( path, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING );
		if( FAILED( hr ) )
			return validationError( L"Nije moguće otvoriti fajl modela", hr );
...
LRESULT LoadModelDlg::OnCallbackStatus( UINT, WPARAM wParam, LPARAM, BOOL& bHandled )
{
	...
	if( FAILED( hr ) )
	{
		LPCTSTR failMessage = L"Greška pri učitavanju modela";
