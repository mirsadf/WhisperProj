...
void CaptureDlg::onDeviceRefresh()
{
	...
	if( len == 0 )
	{
		MessageBox( L"Ni jedan uredjaj za snimanje nije nadjen.\nAko imate usb mikrofon povezite ga na pc,\ni pritisnite “refresh” dugme.",
			L"Capture uredjaji", MB_OK | MB_ICONWARNING );
	}
	else
	{
		const char* suffix = ( len != 1 ) ? "s" : "";
		str.Format( L"Detektovano %zu uredjaja%S.", len, suffix );
		MessageBox( str, L"Capture uredjaji", MB_OK | MB_ICONINFORMATION );
	}
}
...
void CaptureDlg::onBrowseResult()
{
	...
	LPCTSTR outputFilters = L"Tekstualni fajlovi (*.txt)\0*.txt\0\0";
	CString path;
...
void CaptureDlg::setPending( bool nowPending )
{
	...
	else
	{
		...
		btnRunCapture.SetWindowText( L"Snimaj" );
...
void CaptureDlg::onRunCapture()
{
	...
	{
		showError( L"Izaberite uredjaj za snimanje", S_FALSE );
		return;
	}
	...
	if( (uint32_t)threadState.flags & (uint32_t)eTextFlags::Save )
	{
		...
		{
			showError( L"Izaberite izlazni tekstualni fajl", S_FALSE );
			return;
		}
...
inline HRESULT CaptureDlg::runCapture()
{
	...
	if( flags & (uint32_t)eTextFlags::Save )
	{
		...
		if( FAILED( hr ) )
		{
			threadState.errorMessage = L"Ne mogu da kreiram izlazni tekstualni fajl";
			return hr;
		}
		if( append )
		{
...
{
	reportError( m_hWnd, text, L"Snimanje nije uspelo", hr );
}
...
LRESULT CaptureDlg::onThreadQuit( UINT nMessage, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if( FAILED( hr ) )
	{
		LPCTSTR failMessage = L"Snimanje nije uspelo";
